...
function App() {
  ...
  const [messageLog, setMessageLog] = useState("No data")
  ...
  useEffect(() => {
    ...
    stompClient.connect({}, () => {
      ...
      stompClient.subscribe("/topic/connect", (message: any) => {
        setMessageLog(message.body.slice(12, -2))
      });
  ...
  return (
    <>
      {severStatus == true ? <h1>Connected to server</h1> : <div><h1>Not connected to server</h1> <button onClick={() => setAttemptReconnect(attemptReconnect + 1)}>Reconnect</button></div>}
      <h1>{messageLog}</h1>
    </>
  )
}
